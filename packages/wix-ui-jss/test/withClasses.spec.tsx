...
const Component: React.FC<Classes> = ({ classes, children }) => (
  <div className={classes.someClass}>{children}</div>
);
...
const getJssStyleElement = () => {
  ...
  return styleElement && styleElement.hasAttribute('data-jss')
    ? styleElement
    : undefined;
};
...
describe('withClasses', () => {
  ...
  it('should set an id for the component and map it to the style element in the dom', () => {
    ...
    expect(sheetMapper[wrapper.first().instance().id].styleElement).toBe(
      getJssStyleElement(),
    );
  });
  ...
  it('should update the style element when the theme changes, and remove the old style element', () => {
    ...
    const updatedNumberOfDomStyleElements = document.querySelectorAll('style')
      .length;
