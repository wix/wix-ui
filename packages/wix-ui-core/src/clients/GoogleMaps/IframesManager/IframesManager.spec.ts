...
import {
  getIframes,
  getIframeWithLangAndApiKey,
  isIframeVisible,
} from '../IframeTestUtils';
let iframesManager: IframesManager;

const URL_WITH_API_KEY =
  'https://maps.googleapis.com/maps/api/js?libraries=places&key=api-key&language=en&callback=googleReady';
const URL_WITH_CLIENT_ID =
  'https://maps.googleapis.com/maps/api/js?libraries=places&client=client-id&language=en&callback=googleReady';
...
describe('IframesManager', () => {
  ...
  describe('URLs', () => {
    ...
    it('should generate URL using clientId', () => {
      const url = IframesManager.generateUrl({
        clientId: 'client-id',
        lang: 'en',
      });
