          ...
          predict: jest.fn().mockImplementation(async (request: any) => {
            if (
              ...
              (request.countryCodes &&
                request.countryCodes[0] !== predictRequestMock.countryCodes[0])
            ) {
              predictSucceeded = false;
            }
            return predictSucceeded
              ? Promise.resolve(successfulPredictResponseMock)
              : Promise.reject('PredictFailed');
          }),
        };
      }),
    ),
      ...
      jest.fn().mockImplementation((headers: any) => {
        ...
        return {
          getPlace: jest.fn().mockImplementation(async (request: any) => {
            ...
            return getPlaceSucceeded
              ? Promise.resolve(successfulGetPlaceResponseMock)
              : Promise.reject('GetPlaceFailed');
          }),
        };
      }),
    ),
