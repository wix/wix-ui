...
describe('ProgressBar', () => {
  ...
  function runTests(createDriver) {
    ...
    describe('Accessability props', () => {
      ...
      describe('Aria live', () => {
        it('should have aria-live value of `off` by default', async () => {
          const driver = createDriver(<LinearProgressBar {...defaultProps} />);
          expect(await driver.getAriaLive()).toBe('off');
        });
        it('should allow passing `aria-live` prop', async () => {
          type ariaLiveType = 'off' | 'polite' | 'assertive';
          const ariaLiveValues: ariaLiveType[] = ['off', 'assertive', 'polite'];
          for (const ariaLiveValue of ariaLiveValues) {
            const driver = createDriver(
              <LinearProgressBar {...defaultProps} aria-live={ariaLiveValue} />,
            );
            expect(await driver.getAriaLive()).toBe(ariaLiveValue);
          }
        });
      });
    });
