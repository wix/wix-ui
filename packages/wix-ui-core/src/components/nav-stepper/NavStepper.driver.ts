...
export class NavStepperDriver implements BaseDriver {
  ...
  ) => this.styleUtil.hasStyleState(step, state);
  private readonly stepAt = (index: number) =>
    this.element.getElementsByTagName('li')[index];
  ...
  isStepActive = (index: number) =>
    this.hasStyleState(this.stepAt(index), 'active');

  /** checks if a step is disabled */
  isStepDisabled = (index: number) =>
    this.hasStyleState(this.stepAt(index), 'disabled');

  /** checks if a step is visited */
  isStepVisited = (index: number) =>
    this.hasStyleState(this.stepAt(index), 'visited');
...
export const navStepperDriverFactory: DriverFactory<NavStepperDriver> = ({
  element,
}: ComponentFactory): NavStepperDriver => new NavStepperDriver(element);
