...
describe('TimePicker', () => {
  ...
  describe('onChange prop', () => {
    ...
    it('should be called with null when deleting a valid time to "--:--"', () => {
      const onChange = jest.fn();
      const driver = createDriver(
        <TimePicker value={SOME_VALUE} onChange={onChange} />,
      );
      driver.focus();
  ...
  describe('useNativeInteraction prop', () => {
    ...
    it('should send empty string value instead of null value', () => {
      const driver = createDriver(
        <TimePicker useNativeInteraction value={null} />,
      );
    ...
    it('should crop the time value to HH:MM format', () => {
      const driver = createDriver(
        <TimePicker useNativeInteraction value="12:12:12" />,
      );
  ...
  describe('useAmPm prop', () => {
    ...
    it('should display time in 24-hour format when AmPmOptions.None', () => {
      const driver = createDriver(
        <TimePicker value={SOME_VALUE} useAmPm={AmPmOptions.None} />,
      );
      expect(driver.getValue()).toEqual(SOME_VALUE);
    });
    ...
    it('should display time in lowercase 12-hour format when AmPmOptions.Lowercase', () => {
      const driver = createDriver(
        <TimePicker value={SOME_VALUE} useAmPm={AmPmOptions.Lowercase} />,
      );
      expect(driver.getValue()).toEqual(
        convertToAmPm({
          value: SOME_VALUE,
          strings: AmPmStrings[AmPmOptions.Lowercase],
        }),
      );
    });
    ...
    it('should display time in uppercase 12-hour format when AmPmOptions.Uppercase', () => {
      const driver = createDriver(
        <TimePicker value={SOME_VALUE} useAmPm={AmPmOptions.Uppercase} />,
      );
      expect(driver.getValue()).toEqual(
        convertToAmPm({
          value: SOME_VALUE,
          strings: AmPmStrings[AmPmOptions.Uppercase],
        }),
      );
    });
    ...
    it('should display time in capitalized 12-hour format when AmPmOptions.Capitalized', () => {
      const driver = createDriver(
        <TimePicker value={SOME_VALUE} useAmPm={AmPmOptions.Capitalized} />,
      );
      expect(driver.getValue()).toEqual(
        convertToAmPm({
          value: SOME_VALUE,
          strings: AmPmStrings[AmPmOptions.Capitalized],
        }),
      );
    });
  });
  ...
  describe('value prop', () => {
    ...
    it('should set the value when a new value is sent', () => {
      ...
      > {
        ...
        render() {
          return (
            <TimePicker ref={this.timePickerRef} value={this.state.value} />
          );
        }
      }
    ...
    describe('invalid values', () => {
      ...
      beforeEach(
        () =>
          (stub = jest.spyOn(console, 'error').mockImplementation(() => null)),
      );
      ...
      it('should send empty string value instead of invalid value when using native interaction', () => {
        const driver = createDriver(
          <TimePicker useNativeInteraction value="a" />,
        );
        expect(driver.getValue()).toEqual('');
      });
    });
  ...
  describe('style prop', () => {
    it('should set inline style on input', () => {
      ...
      expect(driver.getInlineStyle()).toEqual(
        expect.objectContaining(inlineStyle),
      );
    });
  });
  ...
  describe('tickers', () => {
    ...
    it('should NOT render tickers if both tickerUpIcon and tickerDownIcon are not supplied', () => {
      const driver = createDriver(
        <TimePicker tickerDownIcon={tickerDownIcon} />,
      );
  ...
  describe('readOnly props', () => {
    it('should not update value when typing if readOnly', () => {
      const onChangeSpy = jest.fn();
      const driver = createDriver(
        <TimePicker readOnly onChange={onChangeSpy} />,
      );
      driver.focus();
      driver.keyDown('ArrowDown');
      expect(onChangeSpy).not.toHaveBeenCalled();
