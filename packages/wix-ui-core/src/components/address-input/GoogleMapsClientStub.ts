...
export class GoogleMapsClientStub implements GoogleMapsClient {
  ...
  autocomplete(apiKey: string, lang: string, request: string) {
    ...
    return new Promise<Address[]>((resolve, reject) =>
      setTimeout(() => resolve(addresses), delay),
    );
  }
  ...
  geocode(apiKey: string, lang: string, request: string) {
    ...
    return new Promise<Geocode[]>((resolve, reject) =>
      setTimeout(() => resolve(geocode), delay),
    );
  }
  ...
  placeDetails(apiKey: string, lang: string, request: string) {
    ...
    return new Promise<PlaceDetails>((resolve, reject) =>
      setTimeout(() => resolve(placeDetails), delay),
    );
  }
  ...
  static setAddresses(addresses: Address[], addressesDelay: number = 0) {
    GoogleMapsClientStub.addresses = addresses;
    GoogleMapsClientStub.addressesDelay = addressesDelay;
  }

  static setAddressesPromise(addresses: Address[]) {
    let resolve = null,
      reject = null;
  ...
  static setPlaceDetails(
    placeDetails: PlaceDetails,
    placeDetailsDelay: number = 0,
  ) {
