...
const getMockedDirectoryHelper = (rulesDirectory: string) => ({ src, rule }) => helper({ src, rule, rulesDirectory });
const mockedHelper = getMockedDirectoryHelper(path.resolve(__dirname, '../src'));

describe('no-full-wsr-lib', () => {
  const base = path.resolve(__dirname, 'fixtures', 'disableNoFullWsrLubRule');
  let cwd: string;
  ...
  it.only(`should not fail`, () => {
    ...
    const result = mockedHelper({ src, rule });
  ...
  it('should not fix destructured import statement', () => {
    const src = `import { Button, Panel } from 'wix-style-react';`;

    const result = mockedHelper({ src, rule });
    expect(getFixedResult({ src, rule })).toEqual(src);
  });
